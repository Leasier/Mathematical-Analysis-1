...
方便起见，记 $R[a, b]$ 表示在 $[a, b]$ 上 Riemann 可积的函数集合。

### 定积分的性质

#### 定积分的线性性

内容：若 $f, g \in R[a, b]$，则 $\forall f, g \in \mathbb{R}, \alpha f + \beta g \in R[a, b]$，且 $\displaystyle\int_a^b (\alpha f(x) + \beta g(x)) \text{d} x = \alpha \int_a^b f(x) \text{d} x + \beta \int_a^b g(x) \text{d} x$。

证明略。

#### 被积函数的有界性

内容：若 $f \in R[a, b]$，则 $f$ 在 $[a, b]$ 上有界。
...
内容：若 $f \in R[a, b], f \in R[b, c]$，则 $f \in R[a, c]$，且 $\displaystyle\int_a^c f(x) \text{d} x = \int_a^b f(x) \text{d} x + \int_b^c f(x) \text{d} x$。
...
内容：若 $f, g \in R[a, b]$，且 $\forall x \in [a, b], f(x) \leq g(x)$，则 $\displaystyle\int_a^b f(x) \text{d} x \leq \int_a^b g(x) \text{d} x$。

证明：

- 定义 $h(x) = g(x) - f(x)$，则 $h \in R[a, b]$，且 $\displaystyle\int_a^b h(x) \text{d} x = \int_a^b g(x) \text{d} x - \int_a^b f(x) \text{d} x$。
...
方便起见，记 $\omega_i = M_i - m_i, \Omega(f, P) = U(f, P) - L(f, P)$。
...
- (1) 令 $m = \displaystyle\inf_{x \in [a, b]} f(x), M = \sup_{x \in [a, b]} F(x)$。
...
- (1) $\forall \epsilon > 0, \exists P, \text{s.t. } \Omega(f, P) < \epsilon$。
- (2) $\underline{I} = \overline{I}$。
- (3) $\displaystyle\lim_{|P| \to 0} \Omega(f, P) = 0$。
- (4) $f$ 在 $[a, b]$ 上 Riemann 可积。

证明：

- $(1) \Rightarrow (2)$：由 $L(f, P) \leq \underline{I} \leq \overline{I} \leq U(f, P)$ 可知 $0 \leq \overline{I} - \underline{I} \leq \Omega(f, P)$。
- 由 $\forall \epsilon > 0, \exists P, \text{s.t. } \Omega(f, P) < \epsilon$ 可知 $\forall \epsilon > 0, 0 \leq \overline{I} - \underline{I} < \epsilon$。
...
- 故 $\Omega(f, P) \leq (I + \dfrac{1}{3} \epsilon) - (I - \dfrac{1}{3} \epsilon) = \dfrac{2}{3} \epsilon < \epsilon$，而满足 $|P| < \delta$ 的 $P$ 显然存在，遂得证。
...
- 对于 $\dfrac{1}{q} < T$ 的 $x$，其总贡献小于 $T (b - a)$。
- 对于 $\dfrac{1}{q} \geq T$ 的 $x$，其在 $[a, b]$ 中的数量不超过 $\displaystyle\sum_{1 \leq q \leq \frac{1}{T}} (\lfloor \frac{b - a}{q} \rfloor + 1)$，是有限的。设其数量为 $n$，则它们带来的贡献不超过 $n |P|$。

$\forall \epsilon > 0$，令 $T = \dfrac{\epsilon}{2(b - a)}, \delta = \dfrac{\epsilon}{2 \max(n, 1)}$，则 $\forall |P| < \delta, U(f, P) < T(b - a) + n|P| < \epsilon$。

故 $\displaystyle\lim_{|P| \to 0} U(f, P) = 0 = \lim_{|P| \to 0} L(f, P)$，则 $R$ 在 $[a, b]$ 上 Riemann 可积。

------

从例 2 中可以体会到，我们可以将分割划分为“振幅较小”（对应 $\dfrac{1}{q} < T$）和“总长度较小”（对应 $\dfrac{1}{q} \geq T$）的部分，从而说明 $\Omega(f, P)$ 可以“充分小”。下面，我们来将这种“感觉”严格化。

#### 定理（可积的充要条件）

内容：若 $f$ 在 $[a, b]$ 上有界，则 $f \in R[a, b]$ 当且仅当 $\forall \epsilon, \eta > 0, \exists P, \text{s.t. } \displaystyle\sum_{\omega_i \geq \eta} \Delta x_i < \epsilon$。

证明：

- (1) 必要性：由可积的等价命题 (1) 可知 $\forall \epsilon, \eta > 0, \exists P, \text{s.t. } \Omega(f, P) = \displaystyle\sum_{i = 1}^n \omega_i \Delta x_i < \epsilon \eta$。
- 故 $\displaystyle\eta \sum_{\omega_i \geq \eta} \Delta x_i \leq \sum_{\omega_i \geq \eta} \omega_i \Delta x_i \leq \sum_{i = 1}^n \omega_i \Delta x_i < \epsilon \eta \Rightarrow \sum_{\omega_i \geq \eta} \Delta x_i < \epsilon$，遂得证。
- (2) 充分性：令 $m = \displaystyle\inf_{x \in [a, b]} f(x), M = \sup_{x \in [a, b]} F(x)$。
- $\forall \epsilon > 0, \exists P, \text{s.t. } \displaystyle\sum_{\omega_i \geq \frac{\epsilon}{2(b - a)}} \Delta x_i < \frac{\epsilon}{2(M - m + 1)}$。
- 故 $\Omega(f, P) = \displaystyle\sum_{i = 1}^n \omega_i \Delta x_i = \sum_{\omega_i \geq \frac{\epsilon}{2(b - a)}} \omega_i \Delta x_i + \sum_{\omega_i < \frac{\epsilon}{2(b - a)}} \omega_i \Delta x_i \leq (M - m) \cdot \frac{\epsilon}{2(M - m + 1)} + \frac{\epsilon}{2(b - a)} \cdot (b - a) < \epsilon$，遂得证。
...
$$
\begin{aligned}
\Omega(|f|, P) &= \sum_{i = 1}^n \omega_i(|f|, P) \Delta x_i \\
&= \sum_{i = 1}^n \Delta x_i \sup_{x_1, x_2 \in [x_{i - 1}, x_i]} (|f(x_1)| - |f(x_2)|) \\
&\leq \sum_{i = 1}^n \Delta x_i \sup_{x_1, x_2 \in [x_{i - 1}, x_i]} |f(x_1) - f(x_2)| \\
&= \sum_{i = 1}^n \omega_i(f, P) \Delta x_i \\
&= \Omega(f, P)
\end{aligned}
$$
...
$$
\begin{aligned}
\Omega(fg, P) &= \sum_{i = 1}^n \omega_i(fg, P) \Delta x_i \\
...
&\leq \sum_{i = 1}^n \Delta x_i (M_f \omega_i(g, P) + M_g \omega_i(f, P)) \\
...
$$
\begin{aligned}
\Omega(\frac{1}{g}, P) &= \sum_{i = 1}^n \omega_i(\frac{1}{g}, P) \\
...
&= \frac{1}{d^2} \sum_{i = 1}^n \omega_i(g, P) \\
...
[例 3] 已知 $f \in R[a, b]$，且 $f$ 在 $[a, b]$ 外取值为 $0$，求证：$\displaystyle\lim_{h \to 0} \int_a^b |f(x + h) - f(x)| = 0$。

由 $f \in R[a, b]$ 可知其有界为 $M > 0$，令 $g_h(x) = |f(x + h) - f(x)|$。

- _Motivation：$g_h$ 非负且可积性显然，因此我们只需考虑 $U(g_h, P)$。_
- _Motivation：欲考察 $\displaystyle\max_{x \in [l, r]} |f(x + h) - f(x)|$，能否进行一些放缩，使之与 $f$ 的 Darboux 和相关联？_

取 $n \in \mathbb{N}_{\geq 2}$，令 $h = \dfrac{b - a}{2n}$，考察 $g_h$，可以构造下面三个分割：

- $P_0$：$a, a + h, a + 2h, \cdots, b - 2h, b - h, b$。
- $P_1$：$a, a + 2h, \cdots, b - 2h, b$。
- $P_2$：$a, a + h, a + 3h, \cdots, b - 3h, b - h, b$。

接下来考虑 $P_0$ 的相邻标志点间的区间：

- 对于 $[a + 2kh, a + (2k + 1)h], k \in \mathbb{N}_{< n}$，有：

$$
\begin{aligned}
M_{2k + 1}(g_h, P_0) &\leq \max_{x \in [a + 2kh, a + (2k + 1)h]} |f(x + h) - f(x)| \\
&\leq \max_{x_1, x_2 \in [a + 2kh, a + (2k + 2) h]} |f(x_1) - f(x_2)| \\
&= \omega_{k + 1}(f, P_1)
\end{aligned}
$$

- 对于 $[a + (2k + 1)h, a + (2k + 2)h], k \in \mathbb{N}_{< n - 1}$，有：

$$
\begin{aligned}
M_{2k + 2}(g_h, P_0) &\leq \max_{x \in [a + (2k + 1)h, a + (2k + 2) h]} |f(x + h) - f(x)| \\
&\leq \max_{x_1, x_2 \in [a + (2k + 1)h, a + (2k + 3)h]} |f(x_1) - f(x_2)| \\
&= \omega_{k + 2}(f, P_2)
\end{aligned}
$$
...
$$
\begin{aligned}
M_{2n}(g_h, P_0) &\leq \max_{x \in [b - h, b]} |f(x + h) - f(x)| \\
&\leq M
\end{aligned}
$$
...
$$
\begin{aligned}
U(g_h, P_0) &= \sum_{i = 1}^{2n} M_i(g_h, P_0) \cdot h \\
&\leq \frac{1}{2} \sum_{i = 1}^n \omega_i(f, P_1) \cdot 2h + \frac{1}{2} \sum_{i = 2}^n \omega_i(f, P_2) \cdot 2h + Mh \\
&= \frac{1}{2} (\Omega(f, P_1) + \Omega(f, P_2) - \omega_1(f, P_2) - \omega_{n + 1}(f, P_2)) + Mh \\
&\leq \frac{1}{2} (\Omega(f, P_1) + \Omega(f, P_2)) + Mh
\end{aligned}
$$

- 由于 $|P_1| = |P_2| = 2h$，这样的放缩可行。~~因此，现在开始 $\epsilon-\delta$ 全家桶！~~

由 $f \in R[a, b]$ 可知 $\forall \epsilon > 0, \exists \delta > 0, \text{s.t. } \forall |P| < \delta, \Omega(f, P) < \dfrac{1}{2} \epsilon$。

对于确定的 $\epsilon$，令 $\delta_0 = \min\{\dfrac{1}{2} \delta, \dfrac{\epsilon}{6M}, \dfrac{b - a}{4}\}$，则 $\forall h \in (0, \delta_0), 2h < \delta, Mh < \dfrac{1}{6} \epsilon$。

对于确定的 $\epsilon$ 和 $h$，令 $n = \lfloor \dfrac{b - a}{2h} \rfloor, a' = b - 2nh$，同上给出 $[a', b]$ 上的分割 $P_0, P_1, P_2$，则 $\displaystyle\int_{a'}^b g_h(x) \text{d} x = \inf_P U(g_h, P) \leq U(g_h, P_0) \leq \frac{1}{2} (\Omega(f, P_1) + \Omega(f, P_2)) + Mh < \frac{2}{3} \epsilon$。

同时，由于 $\displaystyle\int_a^{a'} g_h(x) \text{d} x \leq 2M \cdot (a' - a) < 2Mh < \frac{1}{3} \epsilon$，可知 $0 \leq \displaystyle\int_a^b g_h(x) \text{d} x = \int_a^{a'} g_h(x) \text{d} x + \int_{a'}^b g_h(x) \text{d} x < \frac{2}{3} \epsilon + \frac{1}{3} \epsilon = \epsilon$。

故 $\displaystyle\lim_{h \to 0^+} \int_a^b |f(x + h) - f(x)| \text{d} x = \lim_{h \to 0^+} \int_a^b g_h(x) \text{d} x = 0$。

同理可知 $\displaystyle\lim_{h \to 0^-} \int_a^b |f(x + h) - f(x)| \text{d} x = 0$，故 $\displaystyle\lim_{h \to 0} \int_a^b |f(x + h) - f(x)| \text{d} x = 0$。

### 积分第一中值定理

内容：

- (1) 若 $f, g$ 在 $[a, b]$ 上可积，且 $g$ 在 $[a, b]$ 上不变号，则 $\exists \eta \in [\displaystyle\inf_{x \in [a, b]} f(x), \sup_{x \in [a, b]} f(x)], \text{s.t. } \int_a^b f(x) g(x) \text{d} x = \eta \int_a^b g(x) \text{d} x$。
- (2) 在 (1) 的基础上，若 $f \in C[a, b]$，则 $\exists \xi \in [a, b], \text{s.t. } \displaystyle\int_a^b f(x) g(x) \text{d} x = f(\xi) \int_a^b g(x) \text{d} x$。

证明：

- (1) 不妨设 $g$ 在 $[a, b]$ 上恒非负，令 $m = \displaystyle\inf_{x \in [a, b]} f(x), M = \sup_{x \in [a, b]} f(x)$。
- 则 $\forall x \in [a, b], m g(x) \leq f(x) g(x) \leq M g(x)$。
- 故 $m \displaystyle\int_a^b g(x) \text{d} x \leq \int_a^b f(x) g(x) \text{d} x \leq M \int_a^b g(x)$，即 $\exists \eta \in [m, M], \text{s.t. } \displaystyle\int_a^b f(x) g(x) \text{d} x = \eta \int_a^b g(x) \text{d} x$。
- (2) 由连续函数的介质性可知 $\exists \xi \in [a, b], \text{s.t. } f(\xi) = \eta$，故 $\exists \xi \in [a, b], \text{s.t. } \displaystyle\int_a^b f(x) g(x) \text{d} x = f(\xi) \int_a^b g(x) \text{d} x$。
