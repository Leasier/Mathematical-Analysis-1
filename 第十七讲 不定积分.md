...
[例 23] 求 $\displaystyle\int \frac{\sqrt{x^2 + 2x + 2}}{x} \text{d} x$。
...
#### 类型三：$\displaystyle\int x^m (a + bx^n)^p \text{d} x \ (a, b \in \mathbb{R}, m, n, p \in \mathbb{Q})$
...
[例 24] 求 $\displaystyle\int \frac{\sqrt{x}}{\sqrt{1 - x^3}} \text{d} x$。
