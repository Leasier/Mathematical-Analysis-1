...
&\int \frac{1}{\sqrt{1 - x^2}} \text{d} x = \arcsin x + C &\quad &\int \frac{1}{\sqrt{x^2 + a^2}} \text{d} x = \ln (x + \sqrt{x^2 + a^2}) + C \\
...
#### $\displaystyle\int R(\sin x, \cos x) \text{d} x$
...
进而可以发现原函数在每个周期 $((2k - 1) \pi, (2k + 1) \pi)$ 内“上升”了 $\dfrac{\pi}{2} - (-\dfrac{\pi}{2}) = \pi$，然后回到原点。
...
只需验证其在 $x_0 = (2k + 1) \pi, k \in \mathbb{Z}$ 处可导且 $F'(x_0) = f(x_0)$ 即可：

- 对于 $x \in (x_0 - 2 \pi, x_0)$，由于 $F$ 在 $[x, x_0]$ 上**连续**、在 $(x, x_0)$ 上**可导**，由 Lagrange 定理可知：

$$
\exists \xi_x \in (x, x_0), \text{s.t. } f(\xi_x) = F'(\xi_x) = \frac{F(x_0) - F(x)}{x_0 - x} = \frac{F(x) - F(x_0)}{x - x_0}
$$

- 由于 $\displaystyle\lim_{x \to x_0^-} \xi_x = x_0$ 且 $f$ 在 $\mathbb{R}$ 上连续，有：
...
- 同理有 $F'_+(x_0) = f(x_0)$，故 $F'(x_0) = f(x_0)$。

综上，$\displaystyle\int f(x) \text{d} x = F(x) + C$。

------

事实上，只要被积函数在“断点”的邻域内连续，依据连续性补全的原函数必然在“断点”处可导。

#### $\displaystyle\int R(x, \sqrt{ax^2 + bx + c}) \text{d} x$
...
[例 22] 求 $\displaystyle\int \frac{\sqrt{x^2 + 2x + 2}}{x} \text{d} x$。
...
#### $\displaystyle\int R(x, \sqrt[n]{\frac{ax + b}{cx + d}}) \text{d} x \ (n \in \mathbb{N}_{> 1}, ad - bc \neq 0)$
...
#### $\displaystyle\int x^m (a + bx^n)^p \text{d} x \ (a, b \in \mathbb{R}, m, n, p \in \mathbb{Q})$
...
- _Chebyshev 证明了除这三种情况外，$\displaystyle\int x^m (a + bx^n)^p \text{d} x$ 的原函数均为非初等函数。_
