...
##### 定理：若 $f$ 在区间 $I$ 上单调，则 $f$ 的间断点至多只有可数个。

证明：

- 不妨设 $f$ 在 $I$ 上单调递增。
- $\forall x \in I$，$f(x^-), f(x^+)$ 均存在且 $f(x^-) \leq f(x^+)$，则 $f$ 的间断点集为 $E = \{x \in (a, b) \mid f(x^-) < f(x^+)\}$。
- _Motivation：我们知道像 $\mathbb{Q}$ 这样的集合是可数的，能不能构造映射？_
- 由**有理数的稠密性**，可知 $\forall x \in E, \exists q(x) \in (f(x^-), f(x^+)) \cap \mathbb{Q}$。
- 由 $\forall x, y \in E \land x < y, q(x) < f(x^+) \leq f(y^-) < q(y)$，可知 $q : E \to \mathbb{Q}$ 是一个**单射**。
- 由 $\mathbb{Q}$ 可数可知 $E$ 至多可数。
...
因此，我们可以定义函数 $g : J \to I$，其中 $\forall y \in J, g(y)$ 取上面提到与之对应的**唯一**的 $x$，这样定义的 $g$ 称作 $f$ 的**反函数**，记作 $g = f^{-1}$。
...
[例 11] 已知 $f \in C(\mathbb{R})$，$\displaystyle\lim_{x \to \infty} f(f(x)) = \infty$，求证：$\displaystyle\lim_{x \to \infty} f(x) = \infty$。

考虑反证法：

- 假设 $\displaystyle\lim_{x \to \infty} f(f(x)) = \infty$ 不成立。
- 于是 $\exists M_0 > 0, \text{s.t. } \forall M_1 > 0, \exists |x^*| > M_1, |f(x^*)| \leq M_0$。
- 由 $f \in C[-M_0, M_0]$ 可知：$\exists M_2 > 0, \text{s.t. } \forall |x| \leq M_0, |f(x)| \leq M_2$。
- 进而由 $|f(x^*)| \leq M_0$ 可知：$|f(f(x^*))| \leq M_2$。
- 总结一下，$\exists M_0 > 0$，存在一个由 $M_0$ 确定的 $M_2 > 0$，使得 $\forall M_1 > 0, \exists |x^*| > M_1, |f(f(x^*))| \leq M_0$。
- 故 $\displaystyle\lim_{x \to \infty} f(f(x)) = \infty$ 不成立，与条件矛盾。
- 故假设不成立，即 $\displaystyle\lim_{x \to \infty} f(x) = \infty$。

[例 12] （24fall mwj 期中 6）

- (1) 已知 $\forall x \in \mathbb{R}, |\sin x| \leq |x|$，求证：$\forall x, y \in \mathbb{R}, |\sin x - \sin y| \leq |x - y|$。
- (2) 若 $f \in C(\mathbb{R})$，且 $\exists a \in (0, 1), \text{s.t. } \forall x, y \in \mathbb{R}, |f(x) - f(y)| \leq a |x - y|$，求证：$\forall x_0 \in \mathbb{R}$，由迭代式 $\forall n \in \mathbb{N}, x_{n + 1} = f(x_n)$ 确定的序列 $\{x_n\}$ 收敛。
- (3) 若 $a \in (0, \dfrac{1}{2})$，用 (1)(2) 中的结论证明：$\forall b \in \mathbb{R}$，以 $x$ 为未知数的方程 $x^3 - bx^2 + x - b = a \sin x$ 有唯一解。

第一问：

- _Motivation：直接利用绝对值不等式显然是不可行的；但 $\sin \Rightarrow$ 和差化积？_
- $\forall x, y \in \mathbb{R}, |\sin x - \sin y| = |2 \sin \dfrac{x - y}{2} \cos \dfrac{x + y}{2}| \leq 2 |\sin \dfrac{x - y}{2}| \leq 2 \cdot |\dfrac{x - y}{2}| = |x - y|$，遂得证。

第二问：

- $\forall n \in \mathbb{N}$，令 $x = x_{n + 1}, y = x_n$，有 $|f(x_{n + 1}) - f(x_n)| \leq a |x_{n + 1} - x_n| \Leftrightarrow |x_{n + 2} - x_{n + 1}| \leq a |x_{n + 1} - x_n| \Rightarrow |x_{n + 1} - x_n| \leq a^n |x_1 - x_0|$。
- _Motivation：我们无从得知 $\{x_n\}$ 的极限；但级数 $\displaystyle\sum_{i = 0}^{+\infty} a^i$ 收敛至 $\dfrac{1}{1 - a}$，可以考虑柯西收敛原理。_
- $\forall \epsilon > 0$，对于一个 $n \in \mathbb{N}$，要使 $\forall p \in \mathbb{N}_+, |x_{n + p} - x_n| < \epsilon$，只要 $|\displaystyle\sum_{i = 0}^{p - 1} (x_{n + i + 1} - x_{n + i})| \leq \sum_{i = 0}^{p - 1} |x_{n + i + 1} - x_{n + i}| \leq \sum_{i = 0}^{p - 1} a^{n + i} |x_1 - x_0| = \dfrac{a^n - a^{n + p}}{1 - a} |x_1 - x_0| < \epsilon$。
- 当 $|x_1 - x_0| = 0$，令 $N = 0$；否则，只要 $\dfrac{a^n}{1 - a} |x_1 - x_0| < \epsilon \Leftrightarrow n > \log_a \dfrac{\epsilon (1 - a)}{|x_1 - x_0|}$，令 $N = \lfloor \log_a \dfrac{\epsilon (1 - a)}{|x_1 - x_0|} \rfloor$。于是 $\forall n > N, p \in \mathbb{N}_+, |x_{n + p} - x_n| < \epsilon$。
- 进而由柯西收敛原理可知 $\{x_n\}$ 收敛。

第三问：

- _Motivation：$b$ 的任意性使我们期待它能作为一个常数出现，这样的话 $f(x) - f(y)$ 时就可以消掉它了。_
- 原方程 $\Leftrightarrow \dfrac{a \sin x}{x^2 + 1} + b = x$。
- _可以发现这样的话左边就是小小的，绝对值不超过 $a$。_
- 令 $f(x) = \dfrac{a \sin x}{x^2 + 1} + b$。
- **Step 1**：证明有解
- $\forall x, y \in \mathbb{R} \land x \neq y, |f(x) - f(y)| = |a (\dfrac{\sin x}{x^2 + 1} - \dfrac{\sin y}{y^2 + 1})| \leq a (\dfrac{|\sin x - \sin y|}{x^2 + 1} + |\sin y| |\dfrac{x^2 - y^2}{(x^2 + 1)(y^2 + 1)}|) \leq a |x - y| (\dfrac{1}{x^2 + 1} + \dfrac{|x + y|}{(x^2 + 1)(y^2 + 1)}) \leq 2a |x - y| < |x - y|$，故 $f$ 符合 (2) 中条件。
- 因此任取 (2) 中序列 $\{x_n\}$，有 $\{x_n\}$ 收敛，设其极限为 $x^*$，易知 $f(x^*) = x^*$，即找到原方程的一个解。
- **Step 2**：证明解唯一
- 对于 $x, y \in \mathbb{R} \land x \neq y$，若 $x, y$ 均为原方程的解，则 $f(x) - f(y) = x - y$，但由 $|f(x) - f(y)| < |x - y|$ 可知 $|f(x) - f(y)| \neq |x - y|$，矛盾。
- 故这样的 $x, y$ 不存在，即解唯一。
- **总结**：综上，原方程有唯一解。
