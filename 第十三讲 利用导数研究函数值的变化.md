...
$\forall x > 0, \dfrac{1}{x} > 0$，则代入例 3 中的不等式可得 $\ln (1 + \dfrac{1}{x}) < \dfrac{1}{x} \Rightarrow f'(x) < 0$。
...
- 故 $\forall x \in \check{U}(x_0), f'(x) (x - x_0) < 0$，而 $f$ 的连续性被可导性蕴含，故由上面的定理可知 $x_0$ 为 $f$ 的严格极大值点。
...
- (2) $x_0$ 为 $f$ 在 $I$ 上**唯一**的临界点。

结论：

- (1) $f''(x_0) < 0 \Rightarrow x_0$ 为 $f$ 的最大值点。
- (2) $f''(x_0) > 0 \Rightarrow x_0$ 为 $f$ 的极小值点。

证明（以 (1) 为例）：

- 由无穷小增量公式可知 $\forall x \in \check{U}^-(x_0, \delta), f'(x) > f'(x_0) = 0, \forall x \in \check{U}^+(x_0, \delta), f'(x) < f'(x_0) = 0$，其中 $\delta > 0$。
- 断言 $f'$ 在 $x_0$ 左右的两个区间内保持定号。
- 假设 $\exists x \in I \cap (-\infty, x_0), \text{s.t. } f'(x) \leq 0$：
- 令 $x' = \max(x_0 - \dfrac{\delta}{2}, x)$，由 Darboux 定理可知 $\exists \xi \in [x, x'], \text{s.t. } f'(\xi) = 0 \in [f'(x), f(x)]$，则 $\xi$ 也为 $f$ 在 $I$ 上的临界点，矛盾。
- $\forall x \in I \cap (-\infty, x_0), f'(x) > 0$，同理有 $\forall x \in I \cap (x_0, +\infty), f'(x) < 0$。
- 因此 $f$ 在 $I \cap (-\infty, x_0)$ 上单调递减、在 $I \cap (x_0, +\infty)$ 上单调递增，故 $x_0$ 为 $f$ 的最大值点。
